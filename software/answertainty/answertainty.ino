...
void loop() {
  ...
  cvRateSlow = map(analogRead(pot), 0, 1023, 2, 99); // Low needs to be even, high needs to be odd. These figures can be set as you wish, to define rate of change
  if ( (cvRateSlow % 2) == 0) {
  ...
  gateTest = random(map(analogRead(pot), 0, 1023, 1700, 10)); // You can experiment with the last two figures for different rates of gate state changes. Maybe try a logarithmic curve.
